...
resource "aws_s3_bucket" "bucket" {
  ...
  bucket = "berlin-weather-${var.bucket_name[count.index]}-${random_string.bucket.result}"
  acl    = var.acl
  ...
  lifecycle_rule {
    ...
    enabled = var.lifecycle_status // Required. Specifies lifecycle rule status
    ...
    transition { //Specifies a period in the object's transitions after objecr creation
      days          = var.transition_days.transition1
